...
class TransfoEnc(nn.Module):
    def __init__(self, 
        ...
        logger.info("Loading PLM from {}".format(args.pretrainpath))
        if torch.cuda.device_count() > 1:
            self.encoder = torch.nn.parallel.DataParallel(BertModel.from_pretrained(args.pretrainpath))
        else:
            self.encoder = BertModel.from_pretrained(args.pretrainpath)
...
def do_training(args, device):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.pretrainpath, do_lower_case=True)
    ...
    sample_num = min(len(train_set), 10000)
    small_train = train_set.shuffle(seed=42).select(range(sample_num))
    ...
    encoder = TransfoEnc(n_src_vocab=len(tokenizer.get_vocab()), len_max_seq=args.max_length, d_word_vec=args.W,
                         ...
                         d_model=model_dim, d_inner=args.W, dropout=0.1)
...
def compute_and_print_ranks(sim_fun, statement_vecs, proof_vecs, filename):
    ...
    with open(filename, "w+") as f:
        for i in tqdm(range(n_examples), leave=False):
...
def do_eval(args, device):
    ...
    if args.model.lower().startswith("scratch"):
        ...
        logger.info("Loading simcse tokenizer")
        tokenizer = AutoTokenizer.from_pretrained(args.pretrainpath, do_lower_case=True)
    ...
    # np.savetxt("./statement_vecs".format(args.output), statement_vecs.cpu().numpy())
    # np.savetxt("./proof_vecs".format(args.output), proof_vecs.cpu().numpy())
    ...
    compute_and_print_ranks(encoder.sim, statement_vecs, proof_vecs, "./temp_ranks".format(args.output))
...
if __name__ == "__main__":
    ...
    train_parser.add_argument("--pretrainpath", type=str, default=None)
...
#    train_parser.add_argument("--intermediate-pooling", "-p", type=int, default=None)
    ...
    eval_parser.add_argument("--pretrainpath", type=str, default=None)
